...
if (isset($_POST['reset-password-admin'])) {
    // Validate input
    ...
    $sql = "UPDATE employeeUsers SET employeePassword = ? WHERE employeeEmail = ?";
    ...
    function deleteOTP($conn, $email) {
        $sql = "DELETE FROM employeeotpinfo WHERE email = ?";
        $stmt = $conn->prepare($sql);
    ...
    header("Location: reset-password-admin.php?error=unauthorizedimporper");
    exit();
}
